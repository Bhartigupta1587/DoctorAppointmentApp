...
public class AppointmentController {
	...
	public Appointment createAppointment(@RequestBody AppointmentRequest appointmentObject) {
		...
		if (!doc.getAppointments().isEmpty()) {
			// Check if the appointment exists with overlapping time for same patient
			// doctor.
			// This will avoid patient booking multiple overlapping appointments.
			boolean appointmentExists = doc.getAppointments().stream()
					.anyMatch(a -> a.getPatient().getId() == appointmentObject.getPatientId()
							&& a.getDate() == appointmentObject.getAppointmentDate()
							&& !AppointmentController.isOverlapping(a.getStartTime(), a.getEndTime(),
									appointmentObject.getStartTime(), appointmentObject.getEndTime()));
	...
	// http://localhost:8080/api/appointments/delete/1
